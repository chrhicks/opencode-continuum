...
describe('task tools', () => {
  ...
  test('create and get tasks', async () => {
    ...
    const getResponse = await runTool<{ task: Task; relationships: unknown[] }>(getTool, { task_id: taskId })
  ...
  test('get task returns error for missing id', async () => {
    const getTool = arbeit_task_get({ directory })
    const response = await runTool<{ task: Task }>(getTool, { task_id: 'tkt-missing' })
  ...
  test('list tasks filters by status and excludes deleted', async () => {
    ...
    const listTool = arbeit_task_list({ directory })
    ...
    const allResponse = await runTool<{ tasks: Task[] }>(listTool, {})
    ...
    const completedResponse = await runTool<{ tasks: Task[] }>(listTool, {
      status: 'completed'
    })
  ...
  test('relationship tools validate and normalize', async () => {
    const createTool = arbeit_task_create({ directory })
    const relationshipAddTool = arbeit_relationship_add({ directory })
    const relationshipRemoveTool = arbeit_relationship_remove({ directory })
    ...
    const duplicate = await runTool<{ relationship: { type: string } }>(relationshipAddTool, {
    ...
    const removeResponse = await runTool<{ removed: boolean }>(relationshipRemoveTool, {
      from_task_id: child.task.id,
      to_task_id: blocker.task.id,
      type: 'blocked_by'
    })
  ...
  test('arbeit_query returns relationship results', async () => {
    ...
    await runTool(relationshipAddTool, {
